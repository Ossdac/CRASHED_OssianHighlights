...
public class World : MonoBehaviour
{
    ...
    private GameObject finalLabyrinth;
    private int[,] map;
    ...
    {
        if (finalLabyrinth == null)
        {
            Instantiate(finalLabyrinth = new GameObject());
        }
        ...
        if(labyrinthReader == null)
        labyrinthReader = labyrinthDesign.GetComponent<LabyrinthReader>();

        map = labyrinthReader.GenerateCubePosition2DArray(chunkSize);
        int mapWidthInChunks = Mathf.FloorToInt((float)(map.GetLength(0)) / chunkSize);
        int mapLengthInChunks = Mathf.FloorToInt((float)map.GetLength(1) / chunkSize);
